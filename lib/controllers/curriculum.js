...
module.exports = {
  ...
  getCourses: async (req, res, next) => {
    try {
      let { plan } = req.params;
      let courses = await CURRICULUM.findAll({
        raw: true,
        where: { major: { [Op.in]: [plan, 'general'] } },
        attributes: ['code', 'name', 'class', 'practice', 'year', 'semester'],
        nest: true,
        include: { all: true },
      });
      ...
      for (const s of Object.keys(studies)) {
        for (const k of Object.keys(studies[s])) {
          if (studies[s][k].prc1 || studies[s][k].prc2 || studies[s][k].prc3) {
            console.log({
              c: studies[s][k].code,
              prc1: studies[s][k].prc1,
              prc2: studies[s][k].prc2,
              prc3: studies[s][k].prc3,
            });
          }
        }
      }
  ...
  getAll: async (req, res, next) => {
    try {
      let res = await CURRICULUM.findAll({ raw: true, include: { all: true }, nest: true });
      console.log(res);
    } catch (err) {
      return next(errorHandler(res, err));
    }
